...
class codeDeploy
{
    ...
    private $user = 'sakilu';    // Site username
    private $pass = 'rwfwgqgrgthytjyujyj2324';    // Site password
    private $repo = 'crud';    // repository name
    private $deploy = '';    // directory to deploy repository, we've included a tool to know what is the exact
...
/////////////////////////// STOP EDITING BELOW THIS LINE //////////////////////////////	
    ...
    {
        $this->deploy = realpath(dirname(__FILE__) . '/../crud') . '/';
        $this->log($this->deploy);
    ...
    function get_repo($node = '')
    {
        ...
        curl_setopt($ch, CURLOPT_USERPWD, "sakilu-deploy:$this->pass");
    ...
    function parse_changes($node = '', $message = '')
    {
        ...
        $objects = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($real_src),
            RecursiveIteratorIterator::SELF_FIRST);

        foreach ($objects as $name => $object) {

            // check to see if file is in ignore list
            if (in_array($object->getBasename(), $this->ignore_files)) continue;

            // remove the first '/' if there is one
            $tmp_name = str_replace($real_src, '', $name);
            if ($tmp_name[0] == '/') $tmp_name = substr($tmp_name, 1);
